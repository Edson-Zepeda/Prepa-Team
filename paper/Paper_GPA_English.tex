...
\title{\projectlogos\\[0.2cm]Prediction and Analysis of Academic Performance Using Artificial Intelligence}
...
The recommendation engine uses only actionable or non-sensitive contextual variables: Age, StudyTimeWeekly, Absences, ParentalEducation, Tutoring, ParentalSupport, Extracurricular, Sports, Music, and Volunteering. StudentID, GradeClass, Gender, and Ethnicity are excluded. In the product layer, ParentalEducation is retained only as contextual information and is not recommended directly; ParentalSupport is interpreted as an ordinal qualitative scale of family support, from very low to very high. The simulated interventions include reducing absences, increasing study hours, enabling tutoring, strengthening family support, and enabling a complementary activity. For intensive scenarios, the engine also considers reducing absences to a maximum of 5 and increasing study time up to 20 weekly hours.
...
After hyperparameter tuning, XGBoost reduced its test error compared with the initial configuration, but it still kept a higher RMSE than LinearRegression. This suggests that the model uses its higher flexibility to better fit the training data, but it does not improve generalization in this dataset. The correlation between LinearRegression and XGBoost predictions was 0.9979, with a mean absolute difference of 0.0453. Therefore, XGBoost appears visually very close to the linear model, but it is not the final best model.
...
When Absences was removed, all models lost predictive capacity. In LinearRegression, RMSE increased from 0.1963 to 0.8692. Even with tuned XGBoost, RMSE without absences was 0.8958. This drop confirms that the main signal of the dataset is concentrated in absences.
...
For a moderate-to-high risk case, the system recommended an intensive plan: reduce absences by 10, increase study time up to 20 hours per week, enable tutoring, increase parental support by 1 level, and enable an extracurricular activity. Under this simulation, the estimated GPA increased from 2.2783 to 4.0000, and the probability of good performance increased from 14.85\% to 99.999\%. This result must not be interpreted as a causal guarantee, but as a model simulation for prioritizing actions.
...
The project repository is available at: \url{https://github.com/Edson-Zepeda/Prepa-Team}. The paper figures and tables are generated using \texttt{paper/generate\_paper\_assets.py}.
